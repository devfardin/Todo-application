...
const CloseTask = () => {
  ...
  const close= data?.filter(items=> items?.status=='Close')
  const totalTask=close?.length
  return (
    <div className='w-full'>
      <Title title="Close Tasks" totalTask={totalTask} />
    {
      isPending ? <Loader /> :
        <div className='grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-5'>
          {
            close?.map((task) => <div className='flex flex-col justify-between border border-neutral-600 rounded-md p-4  bg-neutral-700'>
              <div>
