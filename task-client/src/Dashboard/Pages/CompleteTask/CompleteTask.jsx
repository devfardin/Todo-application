...
import { IoArrowDown, IoArrowUpSharp } from 'react-icons/io5'
...
const CompleteTask = () => {
  ...
  const complete= data?.filter(items=> items?.status=='Completed')
  const totalTask=complete?.length
  return (
    <div className='w-full'>
    <Title title="Completed Tasks" totalTask={totalTask}/>
    {
      isPending ? <Loader /> :
        <div className='grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-5'>
          {
            complete?.map((task) => <div className='flex flex-col justify-between border border-neutral-600 rounded-md p-4  bg-neutral-700'>
              <div>
